...
def get_calcium_plots(RWSo):
    ...
    slope, intercept, r, p, se = linregress(RWSoCa['datenum'], RWSoCa['calcium umol/kg'])
    ...
    L0 = (RWSoCa.year <= 2014)
    L1 = (RWSoCa.year >= 2015) & (RWSoCa.year <= 2017)
    L2 = (RWSoCa.year >= 2017)

    fig, axs = plt.subplots(nrows=3, dpi=300, figsize=(10,6), sharex=True)

    slope, intercept, r, p, se = linregress(RWSoCa[L0]['datenum'], RWSoCa[L0]['calcium umol/kg'])

    ax = axs[0]
    sns.regplot(x='datenum', y='calcium umol/kg', data=RWSoCa[L0], ax=ax,
                scatter_kws={"color": "xkcd:grey"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Ca RWS')
    ...
    slope, intercept, r, p, se = linregress(RWSoCa[L1]['datenum'], RWSoCa[L1]['calcium umol/kg'])

    ax = axs[1]
    sns.regplot(x='datenum', y='calcium umol/kg', data=RWSoCa[L1], ax=ax,
                scatter_kws={"color": "xkcd:grey"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Ca RWS')
    ...
    slope, intercept, r, p, se = linregress(RWSoCa[L2]['datenum'], RWSoCa[L2]['calcium umol/kg'])

    ax = axs[2]
    sns.regplot(x='datenum', y='calcium umol/kg', data=RWSoCa[L2], ax=ax,
                scatter_kws={"color": "xkcd:grey"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Ca RWS')
