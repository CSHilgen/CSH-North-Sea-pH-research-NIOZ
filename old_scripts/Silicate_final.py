...
RWSomeanlr = RWSomean.dropna(axis='rows', how='all', subset=['total_silicate'])
slope, intercept, r, p, se = linregress(RWSomeanlr['datenum'], RWSomeanlr['total_silicate'])
...
RWSomean['sc_silicate'] = SF_tools.seasonalcycle_fit(opt_result['x'], RWSomean['datenum']) # sc = seasoncal cycle
slope, intercept, sine_stretch, sine_shift = opt_result['x']
RWSomean['ms_silicate'] = RWSomean['total_silicate'] - RWSomean['sc_silicate'] # ms = minus seasonality
...
slope, intercept, r, p, se = linregress(RWSomean['datenum'], RWSomean['sc_silicate'])
...
ax = axs[1]
RWSomean.plot.scatter("datenum", "total_silicate", ax=ax, c='xkcd:navy', label='Initial Si RWS')
...
slope, intercept, r, p, se = linregress(RWSomean['datenum'], RWSomean['ms_silicate'])
...
ax = axs[2]
sns.regplot(x='datenum', y='ms_silicate', data=RWSomean, ax=ax, ci=99.9,
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Silicate_season_fitting_RWSomean.png")    
plt.show()
...
#%% # Long Term Trend of Silicate RWSomean (splitting up)
...
slope, intercept, r, p, se = linregress(RWSomean[L0]['datenum'], RWSomean[L0]['total_silicate'])

ax = axs[0]
sns.regplot(x='datenum', y='total_silicate', data=RWSomean[L0], ax=ax,
...
slope, intercept, r, p, se = linregress(RWSomean[L1]['datenum'], RWSomean[L1]['total_silicate'])

ax = axs[1]
sns.regplot(x='datenum', y='total_silicate', data=RWSomean[L1], ax=ax,
            scatter_kws={"color": "xkcd:navy"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial SiO$_4^{-4}$ RWS')
...
slope, intercept, r, p, se = linregress(RWSomean[L2]['datenum'], RWSomean[L2]['total_silicate'])

ax = axs[2]
sns.regplot(x='datenum', y='total_silicate', data=RWSomean[L2], ax=ax,
            scatter_kws={"color": "xkcd:navy"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial SiO$_4^{-4}$ RWS')
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Silicate_split_up_RWSomean.png")     
plt.show()
