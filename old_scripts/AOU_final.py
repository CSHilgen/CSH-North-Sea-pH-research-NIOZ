...
RWSomeanlr = RWSomean.dropna(axis='rows', how='all', subset=['aou'])
slope, intercept, r, p, se = linregress(RWSomeanlr['datenum'], RWSomeanlr['aou'])
...
ax.set_title("RWS Apparent Oxygen Utilisation data - North Sea")
ax.grid(alpha=0.3)
ax.set_xlabel("Time (yrs)")
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/AOU_fitting_RWSomean.png")    
plt.show()
...
slope, intercept, r, p, se = linregress(RWSomean[L0]['datenum'], RWSomean[L0]['aou'])

ax = axs[0]
sns.regplot(x='datenum', y='aou', data=RWSomean[L0], ax=ax,
            scatter_kws={"color": "xkcd:spring green"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial AOU RWS')

ax.set_title("RWSo Apparent Oxygen Utilisation data - North Sea")
ax.grid(alpha=0.3)
ax.set_xlabel(None)
ax.set_ylabel('AOU (µmol/kg)')
ax.set_ylim(-150, 100)
...
slope, intercept, r, p, se = linregress(RWSomean[L1]['datenum'], RWSomean[L1]['aou'])
...
slope, intercept, r, p, se = linregress(RWSomean[L2]['datenum'], RWSomean[L2]['aou'])
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/AOU_split_up_RWSomean.png")     
plt.show()
