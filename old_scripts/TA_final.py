...
combinedmeanlr = combinedmean.dropna(axis='rows', how='all', subset=['alkalinity'])
slope, intercept, r, p, se = linregress(combinedmeanlr['datenum'], combinedmeanlr['alkalinity'])
...
combinedmean = combinedmean.dropna(axis='rows', how='all', subset=['normalized_TA'])
...
sns.regplot(x='datenum', y='normalized_TA', data=combinedmean, ax=ax, ci=99.9,
            scatter_kws={"color": "xkcd:grey"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Normalized A$_T$ Combined')
...
sns.regplot(x='datenum', y='normalized_TA', data=combinedmean[L0], ax=ax,
            scatter_kws={"color": "xkcd:grey"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Normalized A$_T$ Combined')
