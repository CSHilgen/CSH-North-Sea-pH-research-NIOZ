...
socatnsmeanlr = socatnsmean.dropna(axis='rows', how='all', subset=['fco2_sea'])
slope, intercept, r, p, se = linregress(socatnsmeanlr['datenum'], socatnsmeanlr['fco2_sea'])
...
ax.grid(b=True, which='minor', color='grey', linestyle='-', alpha=0.1)
ax.grid(b=True, which='major', color='xkcd:dark grey', linestyle='-', alpha=0.2)
ax.set_title('fCO$_2$ sea data - Seasons North Sea')
...
socatnsmean['sc_fco2_sea'] = SF_tools.seasonalcycle_fit_fco2_sea(opt_result['x'], socatnsmean['datenum']) # sc = seasoncal cycle
slope, intercept, sine_stretch, sine_shift = opt_result['x']
socatnsmean['ms_fco2_sea'] = socatnsmean['fco2_sea'] - socatnsmean['sc_fco2_sea'] # ms = minus seasonality
