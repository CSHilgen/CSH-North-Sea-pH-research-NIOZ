...
RWSomeanlr = RWSomean.dropna(axis='rows', how='all', subset=['salinity'])
slope, intercept, r, p, se = linregress(RWSomeanlr['datenum'], RWSomeanlr['salinity'])

ax = axs[0]
ax.scatter('datenum', 'salinity', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
sns.regplot(x='datenum', y='salinity', data=RWSomean, ax=ax,
...
combinedmeanlr = combinedmean.dropna(axis='rows', how='all', subset=['salinity'])
slope, intercept, r, p, se = linregress(combinedmeanlr['datenum'], combinedmeanlr['salinity'])

ax = axs[1]
ax.scatter("datenum", "salinity", c="b", data=glodapnsmean, label='GLODAP', s=20, alpha=0.4)
ax.scatter('datenum', 'salinity', c='xkcd:greenish', data=Cefasmean, label='CEFAS', s=20, alpha=0.4)
ax.scatter('datenum', 'salinity', c='xkcd:pink', data=D366mean, label='D366', s=20, alpha=0.4)
ax.scatter('datenum', 'salinity', c='xkcd:dark orange', data=RWSnmean, label='RWS', s=20, alpha=0.4)
sns.regplot(x='datenum', y='salinity', data=combinedmean, ax=ax,
...
ax = axs[0]
sc = ax.scatter('datenum', 'salinity', c='dayofyear', cmap=cm, vmin=1, vmax=365, data=RWSomean, s=20)
...
ax = axs[1]
sc = ax.scatter('datenum', 'salinity', c='dayofyear', cmap=cm, vmin=1, vmax=365, data=combinedmean, s=20)
...
ax.scatter('dayofyear', 'salinity', c='xkcd:dark orange', data=RWSnmean, label='RWS', s=20, alpha=0.4)
...
ax.scatter('distance_to_shore', 'salinity', c='xkcd:dark orange', data=RWSnmean, label='RWS', s=20, alpha=0.4)
...
#%% # Long Term Trend of Salinity RWSomean

print('Long term trend of Salinity RWSomean')
...
ax.set_title("RWS Salinity data - North Sea")
ax.grid(alpha=0.3)
ax.set_xlabel("Time (yrs)")
ax.set_ylabel('Salinity')
ax.set_ylim(28, 36)
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Salinity_fitting_RWSomean.png")    
plt.show()
...
#%% # Long Term Trend of Salinity RWSomean (splitting up)
...
slope, intercept, r, p, se = linregress(RWSomean[L0]['datenum'], RWSomean[L0]['salinity'])

ax = axs[0]
sns.regplot(x='datenum', y='salinity', data=RWSomean[L0], ax=ax,
            scatter_kws={"color": "xkcd:golden"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial Salinity RWS')
...
slope, intercept, r, p, se = linregress(RWSomean[L1]['datenum'], RWSomean[L1]['salinity'])

ax = axs[1]
sns.regplot(x='datenum', y='salinity', data=RWSomean[L1], ax=ax,
            scatter_kws={"color": "xkcd:golden"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial Salinity RWS')
...
slope, intercept, r, p, se = linregress(RWSomean[L2]['datenum'], RWSomean[L2]['salinity'])

ax = axs[2]
sns.regplot(x='datenum', y='salinity', data=RWSomean[L2], ax=ax,
            scatter_kws={"color": "xkcd:golden"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial Salinity RWS')
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Salinity_split_up_RWSomean.png")     
plt.show()
