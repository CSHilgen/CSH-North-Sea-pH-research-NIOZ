...
RWSomeanlr = RWSomean.dropna(axis='rows', how='all', subset=['oxygen umol/kg'])
slope, intercept, r, p, se = linregress(RWSomeanlr['datenum'], RWSomeanlr['oxygen umol/kg'])

ax = ax
ax.scatter('datenum', 'oxygen umol/kg', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
sns.regplot(x='datenum', y='oxygen umol/kg', data=RWSomean, ax=ax,
...
ax = ax
sc = ax.scatter('datenum', 'oxygen umol/kg', c='dayofyear', cmap=cm, vmin=1, vmax=365, data=RWSomean, s=20)
...
ax = ax
ax.scatter('dayofyear', 'oxygen umol/kg', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
...
ax = ax
ax.scatter('distance_to_shore', 'oxygen umol/kg', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
...
#%% # Long Term Trend of Oxygen RWSomean

print('Long term trend of Oxygen RWSomean')
...
RWSomean['sc_oxygen umol/kg'] = SF_tools.seasonalcycle_fit(opt_result['x'], RWSomean['datenum']) # sc = seasoncal cycle
slope, intercept, sine_stretch, sine_shift = opt_result['x']
RWSomean['ms_oxygen umol/kg'] = RWSomean['oxygen umol/kg'] - RWSomean['sc_oxygen umol/kg'] # ms = minus seasonality
...
sns.regplot(x='datenum', y='oxygen umol/kg', data=RWSomean, ax=ax, ci=99.9,
            scatter_kws={"color": "xkcd:green"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Oxygen RWS')

ax.set_title("RWSo Oxygen data - North Sea")
ax.grid(alpha=0.3)
ax.set_xlabel("Time (yrs)")
ax.set_ylabel('Oxygen (µmol/kg)')
ax.set_ylim(175, 425)
...
slope, intercept, r, p, se = linregress(RWSomean['datenum'], RWSomean['sc_oxygen umol/kg'])
...
ax = axs[1]
RWSomean.plot.scatter("datenum", "oxygen umol/kg", ax=ax, c='xkcd:green', label='Initial Oxygen RWS')
...
slope, intercept, r, p, se = linregress(RWSomean['datenum'], RWSomean['ms_oxygen umol/kg'])
...
ax = axs[2]
sns.regplot(x='datenum', y='ms_oxygen umol/kg', data=RWSomean, ax=ax, ci=99.9,
            scatter_kws={"color": "xkcd:green"}, line_kws={"color": "blue", 'label': f'y = {slope:.1f}x + {intercept:.1e}'}, label='Seasonal corrected Oxygen RWS')
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Oxygen_season_fitting_RWSomean.png")    
plt.show()
...
#%% # Long Term Trend of Oxygen RWSomean (splitting up)
...
slope, intercept, r, p, se = linregress(RWSomean[L0]['datenum'], RWSomean[L0]['oxygen umol/kg'])

ax = axs[0]
sns.regplot(x='datenum', y='oxygen umol/kg', data=RWSomean[L0], ax=ax,
            scatter_kws={"color": "xkcd:green"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.2f}'}, label='Initial Oxygen RWS')
...
slope, intercept, r, p, se = linregress(RWSomean[L1]['datenum'], RWSomean[L1]['oxygen umol/kg'])
...
slope, intercept, r, p, se = linregress(RWSomean[L2]['datenum'], RWSomean[L2]['oxygen umol/kg'])
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Oxygen_split_up_RWSomean.png")     
plt.show()
