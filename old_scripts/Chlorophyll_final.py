...
RWSomeanlr = RWSomean.dropna(axis='rows', how='all', subset=['chlorophyll'])
slope, intercept, r, p, se = linregress(RWSomeanlr['datenum'], RWSomeanlr['chlorophyll'])

ax = ax
ax.scatter('datenum', 'chlorophyll', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
...
ax = ax
sc = ax.scatter('datenum', 'chlorophyll', c='dayofyear', cmap=cm, vmin=1, vmax=365, data=RWSomean, s=20)
...
ax = ax
ax.scatter('dayofyear', 'chlorophyll', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
...
ax = ax
ax.scatter('distance_to_shore', 'chlorophyll', c='xkcd:aqua', data=RWSomean, label='RWS', s=20, alpha=0.4)
...
#%% # Long Term Trend of Chlorophyll RWSomean

print('Long term trend of Chlorophyll RWSomean')
...
RWSomean['sc_chlorophyll'] = SF_tools.seasonalcycle_fit(opt_result['x'], RWSomean['datenum']) # sc = seasoncal cycle
slope, intercept, sine_stretch, sine_shift = opt_result['x']
RWSomean['ms_chlorophyll'] = RWSomean['chlorophyll'] - RWSomean['sc_chlorophyll'] # ms = minus seasonality
...
ax.set_title("RWSo Total Chlorophyll data - North Sea")
ax.grid(alpha=0.3)
ax.set_xlabel("Time (yrs)")
ax.set_ylabel('Chlorophyll')
ax.set_ylim(0, 30)
...
slope, intercept, r, p, se = linregress(RWSomean['datenum'], RWSomean['sc_chlorophyll'])
...
ax = axs[1]
RWSomean.plot.scatter("datenum", "chlorophyll", ax=ax, c='xkcd:brown', label='Initial Chl RWS')
...
slope, intercept, r, p, se = linregress(RWSomean['datenum'], RWSomean['ms_chlorophyll'])
...
ax = axs[2]
sns.regplot(x='datenum', y='ms_chlorophyll', data=RWSomean, ax=ax, ci=99.9,
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Chlorophyll_season_fitting_RWSomean.png")    
plt.show()

# Chlorophyll 2000-2021 
xbegin = 11353
xend = 17876
...
#%% # Long Term Trend of Chlorophyll RWSomean (splitting up)
...
slope, intercept, r, p, se = linregress(RWSomean[L0]['datenum'], RWSomean[L0]['chlorophyll'])

ax = axs[0]
sns.regplot(x='datenum', y='chlorophyll', data=RWSomean[L0], ax=ax,
            scatter_kws={"color": "xkcd:brown"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Chl RWS')
...
slope, intercept, r, p, se = linregress(RWSomean[L1]['datenum'], RWSomean[L1]['chlorophyll'])

ax = axs[1]
sns.regplot(x='datenum', y='chlorophyll', data=RWSomean[L1], ax=ax,
            scatter_kws={"color": "xkcd:brown"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Chl RWS')
...
slope, intercept, r, p, se = linregress(RWSomean[L2]['datenum'], RWSomean[L2]['chlorophyll'])

ax = axs[2]
sns.regplot(x='datenum', y='chlorophyll', data=RWSomean[L2], ax=ax,
            scatter_kws={"color": "xkcd:brown"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial Chl RWS')
...
plt.tight_layout()
plt.savefig("figures/Long_Term_Trends/Chlorophyll_split_up_RWSomean.png")     
plt.show()
