...
socatnsmeanlr = socatnsmean.dropna(axis='rows', how='all', subset=['deltafco2'])
slope, intercept, r, p, se = linregress(socatnsmeanlr['datenum'], socatnsmeanlr['deltafco2'])
...
ax.set_title('ΔfCO$_2$ data - Datasets North Sea')
...
ax.grid(b=True, which='minor', color='grey', linestyle='-', alpha=0.1)
ax.grid(b=True, which='major', color='xkcd:dark grey', linestyle='-', alpha=0.2)
ax.set_title('ΔfCO$_2$ data - Seasons North Sea')
...
ax.set_title('ΔfCO$_2$ data - Regions North Sea')
...
ax.grid(b=True, which='minor', color='grey', linestyle='-', alpha=0.1)
ax.grid(b=True, which='major', color='xkcd:dark grey', linestyle='-', alpha=0.2)
ax.set_title('ΔfCO$_2$ data - Year North Sea')
...
socatnsmean = socatnsmean.dropna(axis='rows', how='all', subset=['deltafco2'])
slope, intercept, r, p, se = linregress(socatnsmean['datenum'], socatnsmean['deltafco2'])
...
opt_result = least_squares(SF_tools.lsq_seasonalcycle_fit, [0.006, 386, 162, 680], 
                           args=(socatnsmean['datenum'], socatnsmean['deltafco2']))
socatnsmean['sc_deltafco2'] = SF_tools.seasonalcycle_fit(opt_result['x'], socatnsmean['datenum']) # sc = seasoncal cycle
slope, intercept, sine_stretch, sine_shift = opt_result['x']
socatnsmean['ms_deltafco2'] = socatnsmean['deltafco2'] - socatnsmean['sc_deltafco2'] # ms = minus seasonality

fig, axs = plt.subplots(nrows=3, dpi=300, figsize=(10,6), sharex=True)

ax = axs[0]
sns.regplot(x='datenum', y='deltafco2', data=socatnsmean, ax=ax, ci=99.9,
            scatter_kws={"color": "xkcd:dark orange"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial ΔfCO$_2$ SOCAT')

ax.set_title("SOCAT ΔfCO$_2$ data - North Sea")
ax.grid(alpha=0.3)
ax.set_xlabel("Time (yrs)")
ax.set_ylabel('ΔfCO$_2$ (uatm)')
ax.set_ylim(-240, 130)
...
slope, intercept, r, p, se = linregress(socatnsmean['datenum'], socatnsmean['sc_deltafco2'])
...
ax = axs[1]
socatnsmean.plot.scatter("datenum", "deltafco2", ax=ax, c='xkcd:dark orange', label='Initial fCO$_2$ sea SOCAT')
...
slope, intercept, r, p, se = linregress(socatnsmean['datenum'], socatnsmean['ms_deltafco2'])
...
ax = axs[2]
sns.regplot(x='datenum', y='ms_deltafco2', data=socatnsmean, ax=ax, ci=99.9,
...
#%% # Long Term Trend of Delta fCO2 socatnsmean (splitting up)
...
slope, intercept, r, p, se = linregress(socatnsmean[L1]['datenum'], socatnsmean[L1]['deltafco2'])

ax = axs[0]
sns.regplot(x='datenum', y='deltafco2', data=socatnsmean[L1], ax=ax,
            scatter_kws={"color": "xkcd:dark orange"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial ΔfCO$_2$ SOCAT')
...
slope, intercept, r, p, se = linregress(socatnsmean[L2]['datenum'], socatnsmean[L2]['deltafco2'])

ax = axs[1]
sns.regplot(x='datenum', y='deltafco2', data=socatnsmean[L2], ax=ax,
            scatter_kws={"color": "xkcd:dark orange"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}'}, label='Initial ΔfCO$_2$ SOCAT')
