...
ax = axs[1]
combinedmean.plot.scatter("datenum", "dic", ax=ax, c='purple', label='DIC data')
combinedmean.plot.scatter('datenum', 'normalized_DIC', ax=ax, c='grey', label='Normalized DIC data')
...
ax.grid(alpha=0.3)
ax.set_xlabel("Time (yrs)")
ax.set_ylabel('Normalized DIC (μmol/kg)')
ax.xaxis.set_major_locator(mdates.YearLocator())
ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
ax.xaxis.set_minor_locator(mdates.MonthLocator())
plt.xticks(rotation=0)
