...
ax = ax
ax.scatter('datenum', 'pH_total', c='xkcd:water blue', data=RWSomeanpH, label='pH RWS')
# ax.plot('datenum', 'pH_total', c='xkcd:dark grey', data=RWSomeanpHyear, label='pH RWS averaged per year')

ax.scatter('datenum', 'pH_total_spectro_out', c='xkcd:cobalt blue', data=RWSnmeanpH, label='pH$_{spectro}$ RWS')
# ax.plot('datenum', 'pH_total_spectro_out', c='xkcd:dark grey', data=RWSnmeanpHyear, label='pH$_{spectro}$ RWS averaged per year')

# sns.regplot(x='datenum', y='pH_total', data=RWSomeanpH[L0], ax=ax, ci=99.9,
#             scatter_kws={"color": "xkcd:water blue"}, line_kws={"color": "blue", 'label': f'y = {slope:.1e}x + {intercept:.1f}', 'linestyle': '--',}, label='pH RWS')
# sns.regplot(x='datenum', y='pH_total', data=RWSomeanpH[L1], ax=ax, ci=99.9,
#             scatter_kws={"color": "xkcd:water blue"}, line_kws={"color": "blue", 'label': f'y = {aslope:.1e}x + {aintercept:.1f}'})
# sns.regplot(x='datenum', y='pH_total', data=RWSomeanpH[L2], ax=ax, ci=99.9,
#             scatter_kws={"color": "xkcd:water blue"}, line_kws={"color": "blue", 'label': f'y = {bslope:.1e}x + {bintercept:.1f}', 'linestyle': 'dotted'})
# sns.regplot(x='datenum', y='pH_total_spectro_out', data=RWSnmean, ax=ax, ci=99.9,
...
plt.tight_layout()
plt.savefig("figures/Final_plots/Figure_2_pH.png")     
plt.show()