...
allparametersdubbel = allparameters
...
Lwinter = (x_plotting >= 355) | (x_plotting <80) # 21 dec - 21 march
Lsummer = (x_plotting >= 172) | (x_plotting <264) # 21 juni - 21 sep
...
plotting = pd.DataFrame(x_plotting, y_plotting)
meansummer = y_plotting[Lsummer].mean()
meanwinter = y_plotting[Lwinter].mean()
...
ax.set_ylabel("pH")
ax.grid(alpha=0.3)
ax.set_title('pH 2018-2021 - North Sea')
...
pH_means = [0.020852, -0.039362, 0, -0.055226, 0.101218, 0.016450]
DIC_means = [0.068504, 0.047667, 0, 0.014621, 0.081137, 0.072883]
TA_means = [0.024332, 0.016452, 0, -0.000851, 0.049899, 0.052710]
T_means = [-0.087555, -0.093091, 0, -0.098677, -0.096777, -0.099764]

labels_try = ['2000-2021', '2000-2005', '2010-2015', '2015-2018', '2018-2021']
pH_means_try = [0.020852, -0.039362, -0.055226, 0.101218, 0.016450]
...
ax.bar(df['periods'], df['TA_means'], width, color='xkcd:red', label='∆pH due to ∆TA') #, yerr=women_std )
ax.bar(df['periods'], df['DIC_means'], width, bottom=TA_means, color='xkcd:purple', label='∆pH due to ∆DIC') #, yerr=women_std )
ax.bar(df['periods'], df['T_means'], width, color='xkcd:pink', label='∆pH due to ∆T') #, yerr=men_std )
ax.plot(labels_try, pH_means_try, '-o', markerfacecolor='none', linestyle='dashed', color='xkcd:dark blue', label='∆pH$_{predicted}$') #, yerr=men_std )
...
plt.tight_layout()
plt.savefig("figures/pH_Hagens&Middelburg_2016/Quantitative_contribution.png")
plt.show()
